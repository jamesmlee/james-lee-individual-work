...
// add the prepared statements that allow us to CRUD rows in the contacts DB table
...
// instantiatie a JdbcTemplate instance to execute stmts above against the DB 
...
// inject the jdbc template   
    ...
    @Override
// @Transactional annotation and Propagation.REQUIRED b/c this method is making two 
// separate calls to the database. The first call inserts the data from our Contact 
// object into a row in the contacts table. The second call asks the database for 
// the value that was just assigned to the new row    
    @Transactional(propagation = Propagation.REQUIRED, readOnly = false)
    public Contact addContact(Contact contact) {
// SQL_INSERT_CONTACT prepared statement above ... order must be kept        
...
// dynamically building a prepared statement. take info from user and replace ? marks
    @Override
...
// ContactMapper implements the RowMapper interface, which has one method, mapRow
    private static final class ContactMapper implements RowMapper<Contact> {
// mapRow converts rows from DB to DTO objects by transferring data from the ResultSet object to a new DTO.
// here, we convert rows from DB to Contact objects

// ResultSet contains the info in one row of the database
// int indicates which row this is, of all the rows returned by the query
        public Contact mapRow(ResultSet rs, int rowNum) throws SQLException {
            Contact contact = new Contact();
// use setters to set all properties on new Contact object based on info in result set.
// can get different types out of result set based on the column
